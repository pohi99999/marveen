...
// inbound). A copilot-engine agent is launched with --allow-all-tools, i.e.
// with NO tool-permission prompt standing between a smuggled instruction and a
// shell command, so the one thing it must not do is receive a stranger's
// payload framed exactly like a teammate's. Deliberately plain text rather than
// the Claude path's UNTRUSTED_PREAMBLE + <untrusted source="..."> tag pair
// (prompt-safety.ts): that framing is Claude-TUI/Claude-prompt specific and
// carries no meaning for the Copilot CLI. Same intent, engine-appropriate form.
const COPILOT_UNTRUSTED_WARNING =
...
export function formatCopilotInboundMessage(
  safeFrom: string,
  content: string,
  ...
  return `${COPILOT_UNTRUSTED_WARNING}\n${envelope}`
}
...
//     an ink-based TUI (both Claude Code and the Copilot CLI are ink) reads it
//     as Enter/submit. A multi-line inter-agent message or kanban card body
...
//
//  2. THE PER-PANE SEND LOCK (DELIVLOCK805). A copilot agent's pane has more
//     than one in-process writer today -- the message router AND schedule-
...
// called with host=null throughout, matching startCopilotAgentProcess), so the
// null host here is what makes it share a lane with the other writers to the
// same local pane -- they resolve host via readAgentRemoteHost, which is null
// for a local agent.
export async function sendPromptToCopilotSession(session: string, text: string): Promise<'sent'> {
  ...
  if (result.failedOpen) {
    ...
    logger.warn({ session }, 'sendPromptToCopilotSession: delivery lock wait budget elapsed; sent WITHOUT the per-pane lock (fail-open) -- a holder may be wedged')
  }
  return 'sent'
}
