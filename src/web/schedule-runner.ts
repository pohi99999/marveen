...
//      next successful fire, so attempt_count never accumulated (closed by
//      upstream's MAX_LOST_REDELIVERIES / decideLostRedeliveryAction below,
//      v1.39.0 merge: the fork's own counter was dropped as a duplicate);
...
// Gaps 1 and 3 get one pure helper each below; the runner wires them in.
...
export function startScheduleRunner(): NodeJS.Timeout {
  ...
  async function runCheck() {
    ...
    for (const row of pendingRows) {
      ...
      if (lostRedeliveryCounts.has(key) && !cronOccursOnLocalDay(taskDef.schedule, now, cronTz)) {
        lostRedeliveryCounts.delete(key)
        deletePendingTaskRetry(row.task_name, row.agent_name)
