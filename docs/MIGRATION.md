...
- `store/config-overrides.json` — dashboard settings overrides (generated by the Settings UI; plain values only, no secrets)
- `.env` — project secrets
...
- [ ] Settings overrides restored: `cat store/config-overrides.json` (if the file was present on the old machine).
